    ...
    public Page<ProductionEntity> getAllProduction(
        ...
        try {
            while (hasMoreData.get()) {
                ...
                // Флаг для отслеживания, были ли получены данные в этой партии
                    ...
                    CompletableFuture<Void> future = CompletableFuture
                            .supplyAsync(() -> {
                                ...
                                try {
                                    ...
                                    if (!hasMoreData.get()) {
                                        log.debug("[Поток: {}] Запрос #{}.{} отменен: загрузка завершена",
                                    ...
                                    if (!items.isEmpty()) {
                                        ...
                                        // Не устанавливаем hasMoreData в false сразу,
                                        // так как другие запросы в этой партии могут иметь данные
                                        return new ArrayList<ProductionItemResponseDto>();
                                    }
                            ...
                            .exceptionally(throwable -> {
                                log.error("❌ [Поток: {}] Необработанная ошибка в запросе #{}.{}: {}",
                                        Thread.currentThread().getName(), currentBatch, requestNumber, throwable.getMessage());
                                return null;
                            });
                ...
                try {
                    ...
                    // После завершения всех запросов в партии проверяем, были ли данные
                    if (!batchHasData.get()) {
                        log.info("✅ ПАРТИЯ #{} не содержала данных. Завершаем загрузку.", currentBatch);
                    ...
                    log.error("❌ Ошибка при выполнении партии #{}: {}", currentBatch,
                            e.getMessage() != null ? e.getMessage() : "Неизвестная ошибка");

                    // Отменяем незавершенные задачи
                    futures.forEach(f -> f.cancel(true));

                    // В случае ошибки прерываем выполнение
                    break;
                }
            }

        } catch (Exception e) {
            log.error("❌ Критическая ошибка в процессе загрузки: {}", e.getMessage(), e);
        } finally {
            ...
            if (totalTime > 0) {
                ...
                if (seconds > 0) {
                    log.info("📊 Средняя скорость: {} записей/сек",
                            Math.round(totalRecords / seconds));
                }
            }
        }
